...
def pad_tensor_to_modulo(img, mod):
    ...
    out_width = ceil_modulo(width, mod)
    return F.pad(
        img, pad=(0, out_width - width, 0, out_height - height), mode="reflect"
    )
...
class InpaintingDatasetSingle(Dataset):
    def __init__(
        self,
        input_image,
        mask_image,
        ...
        self.mask_filenames = [mask_image]
        self.img_filenames = [input_image]
...
class InpaintingDataset(Dataset):
    def __init__(
        self, datadir, img_suffix=".jpg", pad_out_to_modulo=None, scale_factor=None
    ):
        ...
        print(f"Image Mask: {self.mask_filenames}")
        print(f"Image: {self.img_filenames}")
        self.pad_out_to_modulo = pad_out_to_modulo
        self.scale_factor = scale_factor
...
class PrecomputedInpaintingResultsDataset(InpaintingDataset):
    def __init__(
        self, datadir, predictdir, inpainted_suffix="_inpainted.jpg", **kwargs
    ):
        ...
        self.pred_filenames = [
            os.path.join(
                predictdir,
                os.path.splitext(fname[len(datadir) :])[0] + inpainted_suffix,
            )
            for fname in self.mask_filenames
        ]
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.img_filenames = sorted(
            list(
                glob.glob(
                    os.path.join(self.indir, "**", f"*{img_suffix}"), recursive=True
                )
            )
        )
        self.pad_out_to_modulo = pad_out_to_modulo
