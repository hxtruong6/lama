...
def get_activations(
    files, model, batch_size=50, dims=2048, cuda=False, verbose=False, keep_size=False
):
...
def calculate_activation_statistics(
    ...
    """
    act = get_activations(
        files, model, batch_size, dims, cuda, verbose, keep_size=keep_size
    )
...
def _compute_statistics_of_images(
    images, model, batch_size, dims, cuda, keep_size=False
):
    if isinstance(images, list):  # exact paths to files are provided
        m, s = calculate_activation_statistics(
            images, model, batch_size, dims, cuda, keep_size=keep_size
        )
...
def calculate_fid_given_images(
    images, batch_size, cuda, dims, use_globals=False, keep_size=False
):
    if use_globals:
        global FID_MODEL  # for multiprocessing

    for imgs in images:
        if isinstance(imgs, list) and isinstance(
    ...
    m1, s1 = _compute_statistics_of_images(
        images[0], model, batch_size, dims, cuda, keep_size=False
    )
    m2, s2 = _compute_statistics_of_images(
        images[1], model, batch_size, dims, cuda, keep_size=False
    )
    fid_value = calculate_frechet_distance(m1, s1, m2, s2)
    return fid_value
