...
class PairwiseScore(EvaluatorScore, ABC):
    ...
    def get_value(self, groups=None, states=None):
        ...
        """
        individual_values = (
            torch.cat(states, dim=-1).reshape(-1).cpu().numpy()
...
class LPIPSScore(PairwiseScore):
    def __init__(self, model="net-lin", net="vgg", model_path=None, use_gpu=True):
        ...
        self.score = PerceptualLoss(
            model=model, net=net, model_path=model_path, use_gpu=use_gpu, spatial=False
        ).eval()
        self.reset()
...
class FIDScore(EvaluatorScore):
    ...
    def get_value(self, groups=None, states=None):
        ...
        activations_pred, activations_target = (
            zip(*states)
            if states is not None
        ...
        if groups is None:
            ...
            for label, index in grouping.items():
                if len(index) > 1:
                    group_distance = calculate_frechet_distance(
                        activations_pred[index], activations_target[index], eps=self.eps
                    )
...
class SegmentationAwareScore(EvaluatorScore):
    def __init__(self, weights_path):
        ...
        self.segm_network = SegmentationModule(
            weights_path=weights_path, use_default_normalization=True
        ).eval()
    ...
    def forward(self, pred_batch, target_batch, mask):
        pred_segm_flat = (
            self.segm_network.predict(pred_batch)[0]
            .view(pred_batch.shape[0], -1)
            .long()
            .detach()
            .cpu()
            .numpy()
        ...
        target_segm_flat = (
            self.segm_network.predict(target_batch)[0]
            .view(pred_batch.shape[0], -1)
            .long()
            .detach()
            .cpu()
        ...
        for cur_pred_segm, cur_target_segm, cur_mask in zip(
            pred_segm_flat, target_segm_flat, mask_flat
        ):
            cur_target_class_freq_total = np.bincount(
                cur_target_segm, minlength=NUM_CLASS
            )[None, ...]
            cur_target_class_freq_mask = np.bincount(
                cur_target_segm[cur_mask], minlength=NUM_CLASS
            )[None, ...]
            cur_pred_class_freq_mask = np.bincount(
                cur_pred_segm[cur_mask], minlength=NUM_CLASS
            )[None, ...]
...
def distribute_values_to_classes(target_class_freq_by_image_mask, values, idx2name):
    assert (
        target_class_freq_by_image_mask.ndim == 2
        and target_class_freq_by_image_mask.shape[0] == values.shape[0]
    )
...
def get_segmentation_idx2name():
    return {
        i - 1: name
        for i, name in segm_options["classes"]
        .set_index("Idx", drop=True)["Name"]
        .to_dict()
        .items()
    }
    ...
    def get_value(self, groups=None, states=None):
        ...
        total_results = {
            ...
            **distribute_values_to_classes(
                target_class_freq_by_image_mask, individual_values, self.segm_idx2name
            ),
        }
        ...
        for label, index in grouping.items():
            ...
            group_results[label] = {
                ...
                "std": group_scores.std(),
                **distribute_values_to_classes(
                    group_class_freq, group_scores, self.segm_idx2name
                ),
            }
        return total_results, group_results
...
class SegmentationClassStats(SegmentationAwarePairwiseScore):
    ...
    def get_value(self, groups=None, states=None):
        ...
        target_class_freq_by_image_total_marginal = (
            target_class_freq_by_image_total.sum(0).astype("float32")
        )
        target_class_freq_by_image_total_marginal /= (
            target_class_freq_by_image_total_marginal.sum()
        )

        target_class_freq_by_image_mask_marginal = target_class_freq_by_image_mask.sum(
            0
        ).astype("float32")
        target_class_freq_by_image_mask_marginal /= (
            target_class_freq_by_image_mask_marginal.sum()
        )

        pred_class_freq_diff = (
            pred_class_freq_by_image_mask - target_class_freq_by_image_mask
        ).sum(0) / (target_class_freq_by_image_mask.sum(0) + 1e-3)
        ...
        grouping = get_groupings(groups)
        for label, index in grouping.items():
            group_target_class_freq_by_image_total = target_class_freq_by_image_total[
                index
            ]
            group_target_class_freq_by_image_mask = target_class_freq_by_image_mask[
                index
            ]
            group_pred_class_freq_by_image_mask = pred_class_freq_by_image_mask[index]

            group_target_class_freq_by_image_total_marginal = (
                group_target_class_freq_by_image_total.sum(0).astype("float32")
            )
            group_target_class_freq_by_image_total_marginal /= (
                group_target_class_freq_by_image_total_marginal.sum()
            )

            group_target_class_freq_by_image_mask_marginal = (
                group_target_class_freq_by_image_mask.sum(0).astype("float32")
            )
            group_target_class_freq_by_image_mask_marginal /= (
                group_target_class_freq_by_image_mask_marginal.sum()
            )

            group_pred_class_freq_diff = (
                group_pred_class_freq_by_image_mask
                - group_target_class_freq_by_image_mask
            ).sum(0) / (group_target_class_freq_by_image_mask.sum(0) + 1e-3)

            cur_group_results = dict()
            cur_group_results.update(
                {
                    f"total_freq/{self.segm_idx2name[i]}": v
...
class SegmentationAwareLPIPS(SegmentationAwarePairwiseScore):
    def __init__(
        self, *args, model="net-lin", net="vgg", model_path=None, use_gpu=True, **kwargs
    ):
        super().__init__(*args, **kwargs)
        self.score_impl = PerceptualLoss(
            model=model, net=net, model_path=model_path, use_gpu=use_gpu, spatial=False
        ).eval()

    def calc_score(self, pred_batch, target_batch, mask):
        return (
            self.score_impl(pred_batch, target_batch).flatten().detach().cpu().numpy()
        )
...
class SegmentationAwareFID(SegmentationAwarePairwiseScore):
    ...
    def get_value(self, groups=None, states=None):
        ...
        total_results = {
            "mean": calculate_frechet_distance(
                activations_pred, activations_target, eps=self.eps
            ),
            "std": 0,
            **self.distribute_fid_to_classes(
                target_class_freq_by_image_mask, activations_pred, activations_target
            ),
        }
        ...
        for label, index in grouping.items():
            if len(index) > 1:
                ...
                group_results[label] = {
                    "mean": calculate_frechet_distance(
                        group_activations_pred, group_activations_target, eps=self.eps
                    ),
                    "std": 0,
                    **self.distribute_fid_to_classes(
                        group_class_freq,
    ...
    def distribute_fid_to_classes(
        self, class_freq, activations_pred, activations_target
    ):
