...
def update_running_average(
    result: nn.Module, new_iterate_model: nn.Module, decay=0.999
):
    with torch.no_grad():
...
class BaseInpaintingTrainingModule(ptl.LightningModule):
        ...
        **kwargs,
        ...
        if not predict_only:
            ...
            if extra_val:
                ...
                self.extra_evaluators = nn.ModuleDict(
                    {k: make_evaluator(**self.config.evaluator) for k in extra_val}
                )
            else:
        ...
        return [
            dict(
                optimizer=make_optimizer(
                    self.generator.parameters(), **self.config.optimizers.generator
                )
            ),
            dict(
                optimizer=make_optimizer(
                    discriminator_params, **self.config.optimizers.discriminator
                )
            ),
        ]
    ...
    def val_dataloader(self):
        ...
        if extra_val:
            res += [
                make_default_val_dataloader(**extra_val[k])
                for k in self.extra_val_titles
            ]
    ...
    def training_step(self, batch, batch_idx, optimizer_idx=None):
        ...
        return self._do_step(
            batch, batch_idx, mode="train", optimizer_idx=optimizer_idx
        )
    ...
    def training_step_end(self, batch_parts_outputs):
        if (
            ...
            >= self.last_generator_averaging_step + self.average_generator_period
        ):
            if self.generator_average is None:
                self.generator_average = copy.deepcopy(self.generator)
            else:
                update_running_average(
                    self.generator_average,
                    self.generator,
                    decay=self.generator_avg_beta,
                )
        ...
        full_loss = (
            ...
            )  # loss is not tensor when no discriminator used
    ...
    def validation_epoch_end(self, outputs):
        ...
        # standard validation
        val_evaluator_states = [
            s["val_evaluator_state"] for s in outputs if "val_evaluator_state" in s
        ]
        val_evaluator_res = self.val_evaluator.evaluation_end(
        ...
        test_evaluator_states = [
            s["test_evaluator_state"] for s in outputs if "test_evaluator_state" in s
        ]
        test_evaluator_res = self.test_evaluator.evaluation_end(
            states=test_evaluator_states
        )
        ...
        if self.extra_evaluators:
            for cur_eval_title, cur_evaluator in self.extra_evaluators.items():
                ...
                cur_evaluator_res_df = (
                    pd.DataFrame(cur_evaluator_res).stack(1).unstack(0)
                )
    ...
    def _do_step(
        self, batch, batch_idx, mode="train", optimizer_idx=None, extra_val_key=None
    ):
        ...
        if self.get_ddp_rank() in (None, 0) and (
            batch_idx % self.visualize_each_iters == 0 or mode == "test"
        ):
            if self.config.losses.adversarial.weight > 0:
        ...
        result = dict(
            loss=total_loss, log_info=add_prefix_to_keys(metrics, metrics_prefix)
        )
        if mode == "val":
            ...
            result[
                f"extra_val_{extra_val_key}_evaluator_state"
            ] = self.extra_evaluators[extra_val_key].process_batch(batch)
    ...
    def get_current_generator(self, no_average=False):
        if (
            ...
            and self.average_generator
            and self.generator_average is not None
        ):
    ...
    def store_discr_outputs(self, batch):
        ...
        batch["discr_output_real"] = F.interpolate(
            discr_real_out, size=out_size, mode="nearest"
        )
        batch["discr_output_fake"] = F.interpolate(
            discr_fake_out, size=out_size, mode="nearest"
        )
        batch["discr_output_diff"] = (
            batch["discr_output_real"] - batch["discr_output_fake"]
        )
