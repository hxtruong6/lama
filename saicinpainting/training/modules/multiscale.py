...
class ResNetTail(nn.Module):
    def __init__(
        ...
        for _ in range(out_extra_layers_n):
            out_layers += [
                nn.Conv2d(ngf, ngf, kernel_size=1, padding=0),
                up_norm_layer(ngf),
                up_activation,
            ]
        out_layers += [
...
class MultiscaleResNet(nn.Module):
    ...
    def forward(
        ...
        if smallest_scales_num is None:
            assert len(self.heads) == len(ms_inputs), (
                len(self.heads),
                len(ms_inputs),
                smallest_scales_num,
            )
        ...
        ms_features = [
            cur_head(cur_inp) for cur_head, cur_inp in zip(cur_heads, ms_inputs)
        ]
...
class MultiscaleDiscriminatorSimple(nn.Module):
    ...
    def forward(
        ...
        """
        if smallest_scales_num is None:
            assert len(self.ms_impl) == len(ms_inputs), (
                len(self.ms_impl),
                len(ms_inputs),
                smallest_scales_num,
            )
        ...
        return [
            cur_discr(cur_input)
            for cur_discr, cur_input in zip(
                self.ms_impl[-smallest_scales_num:], ms_inputs
            )
        ]
...
class SingleToMultiScaleInputMixin:
    def forward(self, x: torch.Tensor) -> List:
        ...
        ms_inputs = [
            F.interpolate(
                x,
                size=(orig_height // f, orig_width // f),
                mode="bilinear",
                align_corners=False,
            )
            for f in factors
...
class DiscriminatorMultiToSingleOutputStackedMixin:
    ...
    def forward(self, x):
        ...
        scaled_outs = [outs[0]] + [
            F.interpolate(
                cur_out, size=outs[0].shape[-2:], mode="bilinear", align_corners=False
            )
...
class MultiscaleDiscrSingleInput(
    ...
    DiscriminatorMultiToSingleOutputStackedMixin,
    MultiscaleDiscriminatorSimple,
):
    pass


class MultiscaleResNetSingle(
    GeneratorMultiToSingleOutputMixin, SingleToMultiScaleInputMixin, MultiscaleResNet
):
    pass
