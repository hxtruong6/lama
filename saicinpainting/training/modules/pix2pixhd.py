...
class MultidilatedResnetBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.conv_block = self.build_conv_block(
            dim, padding_type, conv_layer, norm_layer, activation, use_dropout
        )
...
class MultiDilatedGlobalGenerator(nn.Module):
    def __init__(
        ...
        for i in range(n_downsampling):
            ...
            model += deconv_factory(
                deconv_kind, ngf, mult, up_norm_layer, up_activation, max_features
            )
        model += [
            nn.ReflectionPad2d(3),
...
class ConfigGlobalGenerator(nn.Module):
    def __init__(
        ...
        if len(manual_block_spec) == 0:
            manual_block_spec = [
                DotDict(lambda: None, {"n_blocks": n_blocks, "use_default": True})
            ]
...
class GlobalGenerator(nn.Module):
    def __init__(
        ...
        if dilated_blocks_n_start is not None and dilated_blocks_n_start > 0:
            model += make_dil_blocks(
                dilated_blocks_n_start, dilation_block_kind, dilated_block_kwargs
            )
        ...
        for i in range(n_blocks):
            ...
            if (
                i == n_blocks // 2
                and dilated_blocks_n_middle is not None
                and dilated_blocks_n_middle > 0
            ):
                model += make_dil_blocks(
                    dilated_blocks_n_middle, dilation_block_kind, dilated_block_kwargs
                )
        ...
        if dilated_blocks_n is not None and dilated_blocks_n > 0:
            model += make_dil_blocks(
                dilated_blocks_n, dilation_block_kind, dilated_block_kwargs
            )
...
class GlobalGeneratorGated(GlobalGenerator):
    def __init__(self, *args, **kwargs):
        real_kwargs = dict(
            conv_kind="gated_bn_relu", activation=nn.Identity(), norm_layer=nn.Identity
        )
...
class GlobalGeneratorFromSuperChannels(nn.Module):
    def __init__(
        ...
        for i in range(n_blocks2):
            ...
            model += [
                ResnetBlock(
                    dim, padding_type=padding_type, norm_layer=norm_layer, **kwargs
                )
            ]
        ...
        model += [
            nn.Conv2d(
                channels[2 * n_downsampling + 3], output_nc, kernel_size=7, padding=0
            )
        ]
