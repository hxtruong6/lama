...
def get_gauss_kernel(kernel_size, width_factor=1):
    coords = torch.stack(
        torch.meshgrid(torch.arange(kernel_size), torch.arange(kernel_size)), dim=0
    ).float()
    diff = torch.exp(
        -((coords - kernel_size // 2) ** 2).sum(0) / kernel_size / width_factor
    )
    diff /= diff.sum()
    return diff
...
class BlurMask(nn.Module):
    def __init__(self, kernel_size=5, width_factor=1):
        ...
        self.filter.weight.data.copy_(
            get_gauss_kernel(kernel_size, width_factor=width_factor)
        )
...
class EmulatedEDTMask(nn.Module):
    def __init__(self, dilate_kernel_size=5, blur_kernel_size=5, width_factor=1):
        ...
        self.blur_filter.weight.data.copy_(
            get_gauss_kernel(blur_kernel_size, width_factor=width_factor)
        )
...
class PropagatePerceptualSim(nn.Module):
    def __init__(self, level=2, max_iters=10, temperature=500, erode_mask_size=3):
        ...
        if self.do_erode:
            self.erode_mask = nn.Conv2d(
                1, 1, erode_mask_size, padding=erode_mask_size // 2, bias=False
            )
            self.erode_mask.weight.data.fill_(1)

    def forward(self, real_img, pred_img, mask):
        with torch.no_grad():
            real_img = (real_img - IMAGENET_MEAN.to(real_img)) / IMAGENET_STD.to(
                real_img
            )
            real_feats = self.features(real_img)

            vertical_sim = torch.exp(
                -(real_feats[:, :, 1:] - real_feats[:, :, :-1])
                .pow(2)
                .sum(1, keepdim=True)
                / self.temperature
            )
            ...
            mask_scaled = F.interpolate(
                mask, size=real_feats.shape[-2:], mode="bilinear", align_corners=False
            )
            if self.do_erode:
            ...
            for iter_i in range(self.max_iters):
                new_top_knowness = F.pad(
                    cur_knowness[:, :, :-1] * vertical_sim,
                    (0, 0, 1, 0),
                    mode="replicate",
                )
                new_bottom_knowness = F.pad(
                    cur_knowness[:, :, 1:] * vertical_sim,
                    (0, 0, 0, 1),
                    mode="replicate",
                )

                new_left_knowness = F.pad(
                    cur_knowness[:, :, :, :-1] * horizontal_sim,
                    (1, 0, 0, 0),
                    mode="replicate",
                )
                new_right_knowness = F.pad(
                    cur_knowness[:, :, :, 1:] * horizontal_sim,
                    (0, 1, 0, 0),
                    mode="replicate",
                )
            ...
            cur_knowness = F.interpolate(
                cur_knowness, size=mask.shape[-2:], mode="bilinear"
            )
