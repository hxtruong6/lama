...
DROP_RULES = [re.compile(r"_std$", re.I)]
...
def main(args):
    ...
    for orig_fname in glob.glob(args.inglob):
        cur_dirpath = os.path.dirname(
            orig_fname
        )  # remove filename, this should point to "version_0" directory
            ...
            for v in e.summary.value:
                ...
                if cur_group is None:
                    if v.tag not in ignored_events:
                        print(
                            f"WARNING: Could not detect group for {v.tag}, ignoring it"
                        )
                ...
                if cur_writer is None:
                    if args.include_version:
                        cur_outdir = os.path.join(
                            args.outdir, exp_name, f"{subdirname}_{cur_group}"
                        )
                    else:
                ...
                cur_writer.add_scalar(
                    cur_title, v.simple_value, global_step=e.step, walltime=e.wall_time
                )
