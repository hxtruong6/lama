...
from saicinpainting.evaluation.losses.base_loss import (
    SegmentationAwareSSIM,
    SegmentationClassStats,
    SSIMScore,
    LPIPSScore,
    FIDScore,
    SegmentationAwareLPIPS,
...
def main(args):
    ...
    evaluator = InpaintingEvaluator(
        dataset,
        scores=metrics,
        integral_title="lpips_fid100_f1",
    ...
    if enable_segm:
        only_short_results = results[
            [c for c in results.columns if not c[0].startswith("segm_")]
        ].dropna(axis=1, how="all")
        only_short_results.to_csv(
            args.outpath + "_short", sep="\t", float_format="%.4f"
        )

        print(only_short_results)

        segm_metrics_results = (
            results[["segm_ssim", "segm_lpips", "segm_fid"]]
            .dropna(axis=1, how="all")
            .transpose()
            .unstack(0)
        ...
        segm_stats_results.index = pd.MultiIndex.from_tuples(
            n.split("/") for n in segm_stats_results.index
        )
        segm_stats_results = segm_stats_results.unstack(0).reorder_levels(
            [1, 0], axis=1
        )
        segm_stats_results.sort_index(axis=1, inplace=True)
        segm_stats_results.dropna(axis=0, how="all", inplace=True)

        segm_results = pd.concat(
            [segm_metrics_results, segm_stats_results], axis=1, sort=True
        )
