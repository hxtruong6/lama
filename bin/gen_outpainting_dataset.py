...
def main(args):
    try:
        ...
        for in_img in glob.glob(
            os.path.join(args.indir, "**", "*" + args.img_suffix), recursive=True
        ):
            if "mask" in os.path.basename(in_img):
                continue

            out_img_path = os.path.join(
                args.outdir, os.path.splitext(in_img[len(args.indir) :])[0] + ".png"
            )
...
if __name__ == "__main__":
    ...
    aparser.add_argument(
        "--img-suffix", type=str, default=".png", help="Input image extension"
    )
    aparser.add_argument(
        "--expand",
        action="store_true",
