...
def save_global_samples(
    global_mask_fnames,
    mask2real_fname,
    mask2fake_fname,
    out_dir,
    real_scores_by_fname,
    fake_scores_by_fname,
):
    for cur_mask_fname in global_mask_fnames:
        ...
        fake_img = load_image(mask2fake_fname[cur_mask_fname], mode="RGB")[
        ...
        cur_grid = visualize_mask_and_images(
            dict(image=orig_img, mask=mask, fake=fake_img),
            keys=["image", "fake"],
        ...
        cv2.imwrite(
            os.path.join(
                out_dir, os.path.splitext(os.path.basename(cur_mask_fname))[0] + ".jpg"
            ),
            cur_grid,
        )
...
def save_samples_by_real(worst_best_by_real, mask2fake_fname, fake_info, out_dir):
    for real_fname in worst_best_by_real.index:
        ...
        worst_fake_img = load_image(mask2fake_fname[worst_mask_path], mode="RGB")[
            :, : orig_img.shape[1], : orig_img.shape[2]
        ]
        best_mask_img = load_image(best_mask_path, mode="L")[None, ...]
        best_fake_img = load_image(mask2fake_fname[best_mask_path], mode="RGB")[
            :, : orig_img.shape[1], : orig_img.shape[2]
        ]
        ...
        cur_grid = visualize_mask_and_images(
            dict(
                image=orig_img,
                mask=np.zeros_like(worst_mask_img),
                worst_mask=worst_mask_img,
                worst_img=worst_fake_img,
                best_mask=best_mask_img,
                best_img=best_fake_img,
            ),
        ...
        cv2.imwrite(
            os.path.join(
                out_dir, os.path.splitext(os.path.basename(real_fname))[0] + ".jpg"
            ),
            cur_grid,
        )
...
def main(args):
    ...
    worst_best_by_best_worst_score_diff_max_dir = os.path.join(
        args.outpath, "worst_best_by_real", "best_worst_score_diff_max"
    )
    os.makedirs(worst_best_by_best_worst_score_diff_max_dir, exist_ok=True)
    worst_best_by_best_worst_score_diff_min_dir = os.path.join(
        args.outpath, "worst_best_by_real", "best_worst_score_diff_min"
    )
    os.makedirs(worst_best_by_best_worst_score_diff_min_dir, exist_ok=True)
    worst_best_by_real_best_score_diff_max_dir = os.path.join(
        args.outpath, "worst_best_by_real", "real_best_score_diff_max"
    )
    os.makedirs(worst_best_by_real_best_score_diff_max_dir, exist_ok=True)
    worst_best_by_real_best_score_diff_min_dir = os.path.join(
        args.outpath, "worst_best_by_real", "real_best_score_diff_min"
    )
    os.makedirs(worst_best_by_real_best_score_diff_min_dir, exist_ok=True)
    worst_best_by_real_worst_score_diff_max_dir = os.path.join(
        args.outpath, "worst_best_by_real", "real_worst_score_diff_max"
    )
    os.makedirs(worst_best_by_real_worst_score_diff_max_dir, exist_ok=True)
    worst_best_by_real_worst_score_diff_min_dir = os.path.join(
        args.outpath, "worst_best_by_real", "real_worst_score_diff_min"
    )
        ...
        for batch_i, batch in enumerate(dataset):
            ...
            if cur_real_vector is None:
                with torch.no_grad():
                    ...
                    cur_real_vector = (
                        inception_model(in_img)[0].squeeze(-1).squeeze(-1).cpu().numpy()
                    )
                real2vector_cache[orig_img_fname] = cur_real_vector

            pred_img = torch.from_numpy(batch["inpainted"][None, ...]).cuda()
            cur_fake_vector = (
                inception_model(pred_img)[0].squeeze(-1).squeeze(-1).cpu().numpy()
            )
    ...
    real_info = pd.DataFrame(
        data=[
            dict(real_fname=fname, real_score=score)
            for fname, score in zip(orig_fnames, real_scores)
        ]
    )
    real_info.set_index("real_fname", drop=True, inplace=True)

    fake_info = pd.DataFrame(
        data=[
            dict(
                mask_fname=fname,
            ...
            for fname, score in zip(mask_fnames, fake_scores)
        ]
    )
    ...
    fake_stats_by_real = (
        fake_info.groupby("real_fname")["fake_score"]
        .describe()[["mean", "std"]]
        .rename({"mean": "mean_fake_by_real", "std": "std_fake_by_real"}, axis=1)
    )
    fake_info = fake_info.join(
        fake_stats_by_real, on="real_fname", rsuffix="stat_by_real"
    )
    fake_info.drop_duplicates(["fake_fname", "real_fname"], inplace=True)
    fake_info.to_csv(
        os.path.join(latents_dir, "join_scores_table.csv"), sep="\t", index=False
    )

    fake_scores_table = fake_info.set_index("mask_fname")["fake_score"].to_frame()
    real_scores_table = (
        fake_info.set_index("real_fname")["real_score"].drop_duplicates().to_frame()
    )
    ...
    global_worst_masks = (
        fake_info.sort_values("fake_score", ascending=True)["mask_fname"]
        .iloc[: config.take_global_top]
        .to_list()
    )
    global_best_masks = (
        fake_info.sort_values("fake_score", ascending=False)["mask_fname"]
        .iloc[: config.take_global_top]
        .to_list()
    )
    save_global_samples(
    ...
    # grouped by real
    worst_samples_by_real = (
        fake_info.groupby("real_fname")
        .apply(lambda d: d.set_index("mask_fname")["fake_score"].idxmin())
        .to_frame()
        .rename({0: "worst"}, axis=1)
    )
    best_samples_by_real = (
        fake_info.groupby("real_fname")
        .apply(lambda d: d.set_index("mask_fname")["fake_score"].idxmax())
        .to_frame()
        .rename({0: "best"}, axis=1)
    )
    worst_best_by_real = pd.concat(
        [worst_samples_by_real, best_samples_by_real], axis=1
    )

    worst_best_by_real = worst_best_by_real.join(
        fake_scores_table.rename({"fake_score": "worst_score"}, axis=1), on="worst"
    )
    worst_best_by_real = worst_best_by_real.join(
        fake_scores_table.rename({"fake_score": "best_score"}, axis=1), on="best"
    )
    worst_best_by_real = worst_best_by_real.join(real_scores_table)

    worst_best_by_real["best_worst_score_diff"] = (
        worst_best_by_real["best_score"] - worst_best_by_real["worst_score"]
    )
    worst_best_by_real["real_best_score_diff"] = (
        worst_best_by_real["real_score"] - worst_best_by_real["best_score"]
    )
    worst_best_by_real["real_worst_score_diff"] = (
        worst_best_by_real["real_score"] - worst_best_by_real["worst_score"]
    )

    worst_best_by_best_worst_score_diff_min = worst_best_by_real.sort_values(
        "best_worst_score_diff", ascending=True
    ).iloc[: config.take_worst_best_top]
    worst_best_by_best_worst_score_diff_max = worst_best_by_real.sort_values(
        "best_worst_score_diff", ascending=False
    ).iloc[: config.take_worst_best_top]
    save_samples_by_real(
        worst_best_by_best_worst_score_diff_min,
        mask2fake_fname,
        fake_info,
        worst_best_by_best_worst_score_diff_min_dir,
    )
    save_samples_by_real(
        worst_best_by_best_worst_score_diff_max,
        mask2fake_fname,
        fake_info,
        worst_best_by_best_worst_score_diff_max_dir,
    )

    worst_best_by_real_best_score_diff_min = worst_best_by_real.sort_values(
        "real_best_score_diff", ascending=True
    ).iloc[: config.take_worst_best_top]
    worst_best_by_real_best_score_diff_max = worst_best_by_real.sort_values(
        "real_best_score_diff", ascending=False
    ).iloc[: config.take_worst_best_top]
    save_samples_by_real(
        worst_best_by_real_best_score_diff_min,
        mask2fake_fname,
        fake_info,
        worst_best_by_real_best_score_diff_min_dir,
    )
    save_samples_by_real(
        worst_best_by_real_best_score_diff_max,
        mask2fake_fname,
        fake_info,
        worst_best_by_real_best_score_diff_max_dir,
    )

    worst_best_by_real_worst_score_diff_min = worst_best_by_real.sort_values(
        "real_worst_score_diff", ascending=True
    ).iloc[: config.take_worst_best_top]
    worst_best_by_real_worst_score_diff_max = worst_best_by_real.sort_values(
        "real_worst_score_diff", ascending=False
    ).iloc[: config.take_worst_best_top]
    save_samples_by_real(
        worst_best_by_real_worst_score_diff_min,
        mask2fake_fname,
        fake_info,
        worst_best_by_real_worst_score_diff_min_dir,
    )
    save_samples_by_real(
        worst_best_by_real_worst_score_diff_max,
        mask2fake_fname,
        fake_info,
        worst_best_by_real_worst_score_diff_max_dir,
    )
    ...
    overlapping_mask_fname_score_diffs = overlapping_mask_fname_score_diffs[
        overlapping_sort_idx
    ]
...
if __name__ == "__main__":
    ...
    aparser.add_argument(
        "config", type=str, help="Path to config for dataset generation"
    )
    aparser.add_argument(
        "datadir",
        type=str,
    ...
    aparser.add_argument(
        ...
        type=int,
        default=8,
        help="how many processes to use for pair mask mining",
    )
