...
def process_images(src_images, indir, outdir, config):
    if config.generator_kind == "segmentation":
        ...
        mask_generator = MakeManyMasksWrapper(
            MixedMaskGenerator(**config.mask_generator_kwargs), variants_n=variants_n
        )
    else:
    ...
    for infile in src_images:
        try:
            ...
            for cur_mask in src_masks:
                if config.cropping.out_square_crop:
                    (
                        ...
                        cur_mask, min_overlap=config.cropping.crop_min_overlap
                    )
                    cur_mask = cur_mask[crop_top:crop_bottom, crop_left:crop_right]
                    cur_image = image.copy().crop(
            ...
            for i, idx in enumerate(mask_indices):
                ...
                Image.fromarray(
                    np.clip(cur_mask * 255, 0, 255).astype("uint8"), mode="L"
...
def main(args):
    ...
    in_files = list(
        glob.glob(os.path.join(args.indir, "**", f"*.{args.ext}"), recursive=True)
    )
    if args.n_jobs == 0:
...
if __name__ == "__main__":
    ...
    aparser.add_argument(
        "--n-jobs", type=int, default=0, help="How many processes to use"
    )
