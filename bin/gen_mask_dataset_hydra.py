...
def process_images(src_images, indir, outdir, config):
    if config.generator_kind == "segmentation":
        ...
        mask_generator = MakeManyMasksWrapper(
            MixedMaskGenerator(**mask_generator_kwargs), variants_n=variants_n
        )
    else:
...
@hydra.main(
    config_path="../configs/data_gen/whydra", config_name="random_medium_256.yaml"
)
def main(config: OmegaConf):
    ...
    in_files = list(
        glob.glob(
            os.path.join(config.indir, "**", f"*.{config.location.extension}"),
            recursive=True,
        )
    )
    if config.n_jobs == 0:
        ...
        Parallel(n_jobs=config.n_jobs)(
            delayed(process_images)(
                in_files[start : start + chunk_size],
                config.indir,
                config.outdir,
                config,
            )
