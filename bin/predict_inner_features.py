...
from saicinpainting.training.trainers import (
    load_checkpoint,
...
@hydra.main(
    config_path="../configs/prediction", config_name="default_inner_features.yaml"
)
def main(predict_config: OmegaConf):
    try:
        ...
        assert isinstance(
            model, DefaultInpaintingTrainingModule
        ), "Only DefaultInpaintingTrainingModule is supported"
        ...
        with torch.no_grad():
            for img_i in tqdm.trange(len(dataset)):
                ...
                cur_out_fname = os.path.join(
                    ...
                    os.path.splitext(mask_fname[len(predict_config.indir) :])[0],
                )
                ...
                mask[
                    ...
                    :,
                    mask_h // 2
                    - predict_config.hole_radius : mask_h // 2
                    + predict_config.hole_radius,
                    mask_w // 2
                    - predict_config.hole_radius : mask_w // 2
                    + predict_config.hole_radius,
                ] = 1
                ...
                for level_i, level in enumerate(model.generator.model):
                    ...
                    if level_i in predict_config.levels:
                        cur_feats = (
                            torch.cat([f for f in feats if torch.is_tensor(f)], dim=1)
                        ...
                        if predict_config.slice_channels:
                            cur_feats = cur_feats[
                                :, slice(*predict_config.slice_channels)
                            ]
                        ...
                        cv2.imwrite(
                            cur_out_fname + f"_lev{level_i:02d}_norm.png", cur_feat
                        )
