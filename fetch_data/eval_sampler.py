...
list_of_random_val_files = (
    os.path.abspath(".") + "/places_standard_dataset/original/eval_random_files.txt"
)
val_files = [val_files_path + image for image in os.listdir(val_files_path)]

print(
    f"Sampling 30000 images out of {len(val_files)} images in {val_files_path}"
    + f"and put their paths to {list_of_random_val_files}"
)

print(
    "In our paper we evaluate trained models on these 30k sampled (mask,image) pairs in our paper (check Sup. mat.)"
)
